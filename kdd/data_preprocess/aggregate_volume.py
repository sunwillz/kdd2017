...
_kdd_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__name__))))
_data_dir = os.path.join(_kdd_dir, 'datasets/')
path = os.path.join(_data_dir, 'dataSets', 'training/')     # table 6 所在的表的目录
...
def avgVolume(in_file):
    ...
    out_file_name = _data_dir + in_file.split('_')[1] + out_suffix + file_suffix
